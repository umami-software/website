...
If you don't already have a database, you can follow the [Running on DigitalOcean](/docs/v1/running-on-digitalocean) guide
or the [Running on PlanetScale](/docs/v1/running-on-planetscale) guide to get a database up and running.
You can also check out the **Managed databases** section under [Hosting](/docs/v1/hosting).
...
4. Add the required environment variables `DATABASE_URL` to your Vercel project. These values are defined in the
   **Configure Umami** step from [Install](/docs/v1/install).
5. Deploy and visit your application at `<deploy-id>.vercel.app`.
6. Follow the **Getting started** guide starting from the [Login](/docs/v1/login) step and be sure to change the default password.
