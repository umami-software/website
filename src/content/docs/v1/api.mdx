...
```http request
x-umami-api-key: api_eyTMjU2IiwiY...4Q0JDLUhWxnIjoiUE_A
```
...
stats endpoints require sending a `GET` request with the `x-umami-api-key: <token>` header (post v1.26.0) or `umami.auth` authentication cookie.
