...
Building and maintaining a healty, active community of users and developers is essential for open source projects.
...
scaling are all handled by the us.

We decided not to pursue the other paths to monetization because they conflicted with our principles or goals.

**Open Core Model** - We didn't want to restrict functionality or gate certain featgures because we want to create the best product possible.
We want users to enjoy using it and find value from it. Although we are not against
the open core model, it didn't work for us.
...
can be used as many people as possible so that they can then innovate further. We didn't want to restrict the product in any way.
...
service or source of revenue. The support is mainly to service our cloud product.
