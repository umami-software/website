...
- The tracker no longer uses CSS class names to trigger events. It now uses HTML data attributes. See [Track events](/docs/v2/track-events) for more information.
- The methods on the global `umami` JavaScript object have changed. There is now a single `.track()` method and a new way to send event data.
  See [Tracker functions](/docs/v2/tracker-functions) for more information.
- The `TRACKER_SCRIPT_NAME` environment variable no longer appends the `.js` extension to the script name. See [Environment variables](/docs/v2/environment-variables) for more information.
